...
export default function GameCanvas({onExit,player}:{onExit:()=>void;player:any}){
  ...
  const GoalBar=({goals}:{goals:number})=>(
    <div style={{display:"flex",alignItems:"center",gap:3,marginTop:2}}>
      <div style={{width:52,height:4,background:"rgba(255,255,255,0.15)",borderRadius:4,overflow:"hidden"}}>
        <div style={{height:"100%",width:`${Math.min(100,(goals/WIN_GOALS)*100)}%`,background:goals>=WIN_GOALS?"#22c55e":goals>=7?"#f59e0b":"#22ff44",borderRadius:4,transition:"width 0.3s"}}/>
      </div>
      <span style={{fontSize:9,color:"rgba(255,255,255,0.45)",fontWeight:700,fontFamily:"sans-serif"}}>{goals}/{WIN_GOALS}</span>
    </div>
  );
  ...
  if(isPortrait) return(
    <div style={{position:"fixed",inset:0,background:"#0d0b1e",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:20,zIndex:999}}>
      ...
      <div style={{fontSize:22,fontWeight:900,color:"#fff",fontFamily:"sans-serif"}}>Rotate Your Phone</div>
      <button onClick={handleExit} style={{padding:"10px 24px",borderRadius:12,border:"1px solid rgba(255,255,255,0.15)",background:"rgba(255,255,255,0.07)",color:"rgba(255,255,255,0.6)",fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:"sans-serif"}}>Back to Menu</button>
    </div>
  );

  return(
    <div style={{position:"fixed",inset:0,background:"#4a7ec8"}}>
      {/* Scoreboard HUD - dark box like reference */}
      <div style={{position:"absolute",top:8,left:"50%",transform:"translateX(-50%)",zIndex:10,background:"#1a1a1a",border:"3px solid #444",borderRadius:8,padding:"4px 10px",display:"flex",alignItems:"center",gap:5,pointerEvents:"none",fontFamily:"sans-serif"}}>
        <div style={{background:"#111",borderRadius:5,padding:"3px 10px",textAlign:"center"}}>
          <div style={{fontSize:9,fontWeight:800,color:"#999",letterSpacing:1}}>P1</div>
          <div style={{fontSize:26,fontWeight:900,color:"#22ff44",lineHeight:1}}>{score[0]}</div>
          <GoalBar goals={score[0]}/>
        </div>
        <div style={{padding:"0 6px",textAlign:"center"}}>
          <div style={{fontSize:9,color:"rgba(255,255,255,0.3)",letterSpacing:1,fontWeight:700}}>TIME</div>
          <div style={{fontSize:14,fontWeight:900,color:"#fff"}}>{Math.floor(timeLeft/60)}:{String(timeLeft%60).padStart(2,"0")}</div>
          <div style={{fontSize:8,color:"rgba(255,255,255,0.25)"}}>First to {WIN_GOALS}</div>
        </div>
        <div style={{background:"#111",borderRadius:5,padding:"3px 10px",textAlign:"center"}}>
          <div style={{fontSize:9,fontWeight:800,color:"#999",letterSpacing:1}}>CPU</div>
          <div style={{fontSize:26,fontWeight:900,color:"#22ff44",lineHeight:1}}>{score[1]}</div>
          <GoalBar goals={score[1]}/>
        </div>
      </div>

      <button onClick={handleExit} style={{position:"absolute",top:10,left:10,zIndex:10,background:"rgba(0,0,0,0.5)",border:"1px solid rgba(255,255,255,0.2)",color:"#fff",borderRadius:8,padding:"5px 10px",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"sans-serif"}}>✕</button>

      <canvas ref={canvasRef} style={{width:"100%",height:"100%",display:"block"}}/>

      {/* Match Result */}
      {gameOver&&matchResult&&(
        <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:20,backdropFilter:"blur(6px)"}}>
          <div style={{fontSize:56,marginBottom:8}}>{matchResult.outcome==="win"?"🏆":matchResult.outcome==="draw"?"🤝":"💀"}</div>
          <div style={{fontSize:30,fontWeight:900,color:"#fff",marginBottom:4,fontFamily:"sans-serif"}}>{matchResult.outcome==="win"?"You Win!":matchResult.outcome==="draw"?"Draw!":"CPU Wins!"}</div>
          ...
          <div style={{background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:18,padding:"16px 32px",display:"flex",gap:24,marginBottom:12}}>
            <div style={{textAlign:"center"}}><div style={{fontSize:28}}>🪙</div><div style={{fontSize:22,fontWeight:900,color:"#ffd700",fontFamily:"sans-serif"}}>+{matchResult.coinsEarned}</div><div style={{fontSize:10,color:"rgba(255,255,255,0.4)",fontWeight:700,fontFamily:"sans-serif"}}>COINS</div></div>
            <div style={{textAlign:"center"}}><div style={{fontSize:28}}>⭐</div><div style={{fontSize:22,fontWeight:900,color:matchResult.ratingChange>=0?"#22c55e":"#ef4444",fontFamily:"sans-serif"}}>{matchResult.ratingChange>=0?"+":""}{matchResult.ratingChange}</div><div style={{fontSize:10,color:"rgba(255,255,255,0.4)",fontWeight:700,fontFamily:"sans-serif"}}>RATING</div></div>
            <div style={{textAlign:"center"}}><div style={{fontSize:28}}>📊</div><div style={{fontSize:22,fontWeight:900,color:"#a78bfa",fontFamily:"sans-serif"}}>{matchResult.newRating}</div><div style={{fontSize:10,color:"rgba(255,255,255,0.4)",fontWeight:700,fontFamily:"sans-serif"}}>NEW MMR</div></div>
          </div>
          ...
          <button onClick={handleExit} style={{padding:"14px 44px",borderRadius:16,border:"none",background:"linear-gradient(135deg,#6c3fff,#8b5fff)",color:"#fff",fontWeight:900,fontSize:16,cursor:"pointer",fontFamily:"sans-serif",boxShadow:"0 4px 24px rgba(108,63,255,0.5)"}}>Back to Menu</button>
        </div>
      )}

      {/* Controls */}
      <div style={{position:"absolute",bottom:10,left:0,width:"100%",display:"flex",justifyContent:"space-between",padding:"0 14px",zIndex:10}}>
        <div style={{display:"flex",gap:10}}><Btn label="←" prop="left"/><Btn label="→" prop="right"/></div>
        <div style={{display:"flex",gap:10}}><Btn label="👟" prop="kick" col="rgba(160,25,25,0.92)"/><Btn label="↑" prop="jump" col="rgba(20,80,200,0.92)"/></div>
      </div>
    </div>
  );
}
